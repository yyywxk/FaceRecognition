...
/*std::vector<cv::Rect> VideoFaceDetector::face() const
{
	std::vector<cv::Rect> faceRect = m_allFaces;
...
/*std::vector<cv::Point> VideoFaceDetector::facePosition() const
{
	std::vector<cv::Point> facePos;
	for (int i = 0; i < m_faceNum; i++)
	{

		facePos[i].x = (int)(m_facePosition.x / m_scale);
		facePos[i].y = (int)(m_facePosition.y / m_scale);
	}
    return facePos;
}*/
...
int VideoFaceDetector::faceNum() const
{
	return m_faceNum;
}